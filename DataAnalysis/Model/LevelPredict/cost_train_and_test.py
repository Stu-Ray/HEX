...
# 读取数据
data = pd.read_csv('data/processed_data_cost.csv')
...
# 保存scaler参数
joblib.dump(scaler, './data/model_param/ME_cost/scaler.pkl')  # 添加这行代码来保存scaler
# print(number_tensor, number_tensor.shape)
...
# 定义存储结果的CSV文件名
train_csv_filename = './data/train_and_test_result/ME_cost/train_loss_and_acc.csv'
test_csv_filename = './data/train_and_test_result/ME_cost/test_loss_and_acc.csv'
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 5 == 0:
        torch.save(model,f"./data/model_param/ME_cost/model_{epoch + 1}.pkl")
print(f"Total evaluate time: {evaluate_time:.4f},Average evaluate time: {evaluate_time/(num_epochs*test_loader.batch_size):.4f}")

torch.save(model,f"./data/model_param/ME_cost/model_final.pkl.")
...
plt.ylabel('Loss')
plt.legend()
plt.savefig("./images/ME_cost/loss.png",dpi=300)
plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig("./images/ME_cost/test_acc.png",dpi=300)
plt.show()
