...
// 递归遍历整个计划树，并将其信息添加入currentSQLOpts
OperatorInfo* addPlanTree(Plan* topPlan)
{
	...
	if(!exist_opt)
	{
		...
		{
			currentSQLOpts[opt_index].actual_input_rows = topPlan->input_rows; 	// Scan类算子的输入记为表内tuple数
		}
		else
		{
...
getSqlStatementType(const char* query_string_sql) 
{
    ...
    if (strcmp(firstWord, "SET") == 0) 
	{
		free(firstWord);
        return CMD_NOTHING;
    } else if (strcmp(firstWord, "SELECT") == 0) 
	{
		free(firstWord);
        return CMD_SELECT;
...
exec_simple_query(const char *query_string)
{
	...
	{
		// 记录时间信息的文件
		const char* tFilename = "/data/time.csv";

		FILE *tFile = fopen(tFilename, "a+"); 
		...
		{
			fprintf(tFile, "\"%s\",%s,", query_string, databaseName);
		}
		fclose(tFile);
	...
	{ 
		...
		// 判断当前语句是否为新的语句（与上一条语句不重复）
		if(!currentStatement || strcasecmp(currentStatement, query_string) != 0)
		{
			// 若为新的语句，则初始化相关变量
			currentStatement		=	(char*)malloc(strlen(query_string) + 1);	// 当前SQL语句

			min_total_time			=	0.0;		// 连续重复语句中的最短执行时间
			min_expr_time			=	0.0;		// 同一个表达式在不同级别下的最短执行时间
			Reserved2				=	0;			// 保留变量2，用于记录当前正在变更级别的表达式索引——ME
			Reserved3				=	0;			// 保留变量3，用于记录当前正在全局变更的级别——Quartet
			enableOPT3				=	false;		// 是否使用OPT3优化级别
			useTheBest				=	false;		// 是否使用目前理论最优的优化级别

			// updateExprList			=	true;		// 是否要更新表达式EEOP列表的文件（只有当进入新语句或列表更新时为真）

			if(enableExprCount)
			{
				countExpr			=	true;		// 是否要为表达式计数（只有第一次执行新语句才为真）
			}

			// 当前的表达式数量、信息、级别、整体最佳级别、局部最佳级别
			currentExprNum			=	0;
			currentExprList			=	(ExprInfo*)malloc(EXPR_MAX_NUM * sizeof(ExprInfo));
			currentExprJITLevels	=	(int*)malloc(EXPR_MAX_NUM * sizeof(int));
			bestJITLevels			=	(int*)malloc(EXPR_MAX_NUM * sizeof(int));
			bestCurrentJITLevels	=	(int*)malloc(EXPR_MAX_NUM * sizeof(int));

			for(int i=0; i<EXPR_MAX_NUM; i++)
			{
				currentExprJITLevels[i] = 0;
			}
			// 用于记录出现过的表达式级别组合，防止重复执行同一个优化级别组合（目前已无用）
			previousLevelsNum		=	0;
			previousExprJITLevels	=	(long long*)malloc(LEVELS_NUM * sizeof(long long));

			strcpy(currentStatement, query_string);		//	记录当前SQL语句

			currentStatementExecNum	=	0;				//	当前SQL语句重复执行的次数（次数为1，表示连续执行了2次）
			currentLevelsCount		=	0;				//	当前表达式级别组合连续重复执行的次数（用于重复多次求平均）
		}
		else if(!testME)
		{
			countExpr		=	false;
			...
			if(currentStatementExecNum == 1)	// 如果是当前语句第一次重复执行（即第二次执行）
			{
				// 第一次重复执行时，不使用JIT执行优化
				for(int i = 0; i < currentExprNum; i++)
				{
					currentExprJITLevels[i]	=	0;	
				}
			...
			else if(currentLevelsCount >= Reserved1)	// Reserved1表示每个级别组合需要重复执行的次数，若超出次数则选择新的级别组合
			{
				if(enableQuartet && Reserved3 < 3)		// 全局遍历每个级别（0级和3级在后面遍历）
				{
					++Reserved3;
					for(int i = 0; i < currentExprNum; i++)
					{
						currentExprJITLevels[i]	=	(Reserved3) % 3;
					}	
				}
				else if(Reserved2 >= currentExprNum)	// 如果已经遍历了所有需要的组合，则开始使用最优级别组合执行
				{
					useTheBest	=	true;
				}
				else if(enableOPT3)	// 如果需要使用OPT3级别，则每个级别在0和3之间选择
				{
					// 如果是第一次使用OPT3级别，则初始化局部最优级别的记录，防止和非OPT3的局部最优记录冲突
					if(Reserved2 == 0 && currentExprJITLevels[Reserved2] != 3)
					{
						for(int i = 0; i < currentExprNum; i++)
						{
							currentExprJITLevels[i]	=	0;
					...
					// 如果指定位是OPT3级别，则将当前位的级别置0并进入下一位
					if(currentExprJITLevels[Reserved2] == 3)
					{
						currentExprJITLevels[Reserved2]	= 0;

						min_expr_time		=	0.0;

						Reserved2++;

						if(Reserved2 >= currentExprNum)	// 如果超出表达式总数量，说明遍历完毕，使用（局部）最优级别组合执行一次
						{
							useTheBest	=	true;
						}
						else
						{
							currentExprJITLevels[Reserved2]	= 3;
							useTheBest	=	false;
						}
					}
					else
				...
				else if(!enableOPT3) // 如果不使用OPT3级别，则每个级别在0、1、2之间选择
				{
					currentExprJITLevels[Reserved2] += 1;	// 每次为指定位加1

					// 如果超出2则置0并进位，类似三进制，但保持指定位之外的位均为0
					if(currentExprJITLevels[Reserved2] == 3)
					{
						currentExprJITLevels[Reserved2]	=	0;
						...
						// 如果超出表达式总数，说明已经遍历完毕无OPT3的情况，使用局部最优级别组合执行一次，之后开始允许OPT3级别
						if(Reserved2 >= currentExprNum)
						{
							Reserved2		=	0;			// 需要重新遍历有OPT3的组合，因此索引置0
							useTheBest		=	true;
							enableOPT3 		= 	true;
						}
						else
						{
							useTheBest 		= 	false;
							currentExprJITLevels[Reserved2] += 1;
						}
					}	
				}

				if((Reserved2 >= currentExprNum) && useTheBest && enableOPT3)	// 使用（OPT3）局部最优级别组合执行
				{
					for(int i = 0; i < currentExprNum; i++)
					{
						currentExprJITLevels[i]	=	bestCurrentJITLevels[i];
					}	

					enableOPT3	=	false;
				}
				else if((Reserved2 >= currentExprNum) && useTheBest && !enableOPT3)	// 使用（OPT3+无OPT3）全局最优级别组合执行
				{
					for(int i = 0; i < currentExprNum; i++)
					{
						currentExprJITLevels[i]	=	bestJITLevels[i];
					}
				}
				else if(useTheBest)	// 使用（无OPT3）局部最优级别组合执行
				{
					for(int i = 0; i < currentExprNum; i++)	
					{
						currentExprJITLevels[i]	=	bestCurrentJITLevels[i];
		...
		if(testME)
		{
			int	current_query_id = identify_query(query_string);

			FILE *file = fopen(levelsFile, "r"); 
			if (!file) 
			{
				perror("Failed to open file");
			}
			char line[1024];  // 缓存每行数据
			...
			{ 
				...
				while (token) 
				{
					while (isspace((unsigned char)*token)) token++;	// 去除首尾空白字符
				
					if(token_num == 1)
					{
						if(strcasecmp(databaseName, token) != 0)
						{
							match = false;
							break;
						}
					...
					else if(token_num >= 2)
					{
						...
						if (*endptr == '\0') 
						{
							if(token_num == 2)
							{
								if(current_query_id != value)
								{
									match = false;
									break;
								}
							}
							else if(token_num - 2 <= EXPR_MAX_NUM)
							{
								if(value < 0 || value > 3)
								{
									printf("Something went wrong! Invalid Level!");
									match = false;
									break;
								}
								currentExprJITLevels[token_num - 3] = value;
							}
							else
							{
								printf("Something went wrong! Too many Expressions!");
								match = false;
								break;
							}
						}
						...
						{
							match = false;
							printf("Something went wrong! String found instead of numbers!");
						}
					}
	...
	if(enableME && containExpr)
	{
		if(printExpr)
		{
			...
			const char* ecFilename = "/data/Expression/expression_count.csv";

			FILE *ecFile = fopen(ecFilename, "a+"); 
			...
			{
				fprintf(ecFile, "%s,%d,%d\n", databaseName, currentExprNum, exprNum);
			}
		...
		// 	updateExprList			=	false;
		// }

		// ------------------------ Expression Vector ------------------------ 

		if(testME && containExpr)
		{
			const char* exprFilename = "/data/ExprLevel/exprs.csv";

			FILE *exprFile = fopen(exprFilename, "w+"); 
			...
			if(exprFile)
			{
				for(int i=0; i<allExprInfoNum; i++)
				{
					...
					int	eeop_count[(int)EEOP_LAST] = {0};
					...
					if(currentSQLExprs[i].expression_id != 0)
					{
						if(currentSQLExprs[i].op)
						{
							...
							if(currentSQLExprs[i].op->left)
							{
								left_rows	=	currentSQLExprs[i].op->left->actual_output_rows;
							}

							if(currentSQLExprs[i].op->right)
							{
								right_rows	=	currentSQLExprs[i].op->right->actual_output_rows;
							}

							fprintf(exprFile, "%d,%s,%.0lf,%.0lf,%.0lf,%d,%d", 
								currentSQLExprs[i].expression_id, getNodeTagName((NodeTag)currentSQLExprs[i].op->type), 
								currentSQLExprs[i].op->actual_input_rows, currentSQLExprs[i].op->actual_output_rows, currentSQLExprs[i].op->filtered_rows,
								left_rows, right_rows);
							
							for(int j=0; j<currentSQLExprs[i].step_lens; j++)
							{
								eeop_count[(int)currentSQLExprs[i].steps[j] % (int)EEOP_LAST] += 1;
							}

							for(int k=0; k<(int)EEOP_LAST; k++)
							{
								fprintf(exprFile, ",%d", eeop_count[k]);
							}
