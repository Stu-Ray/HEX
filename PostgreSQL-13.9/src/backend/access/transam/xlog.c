...
{
	exprTypes	=	(ExprSteps*)malloc(EXPR_LIST_SIZE * sizeof(ExprSteps));
    
	for(int i=0; i<EXPR_LIST_SIZE; i++)
    {
        exprTypes[i].eIndex          =   i;
        exprTypes[i].expression_id   =   0;
        exprTypes[i].hash            =   0LL;
        exprTypes[i].step_lens       =   0;
        exprTypes[i].next            =   NULL;
    }
