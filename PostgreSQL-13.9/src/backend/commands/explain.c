			   ...
			   const BufferUsage *bufusage)
{
	...
	if(containExpr)
	{
		const char* filename_time = "/data/time.csv";

		FILE *file_time = fopen(filename_time, "a+"); 
		...
		fprintf(file_time, "%d,%.6lf,%.6lf,%.6lf,%.6lf,%.6lf,%.6lf,%.6lf\n", pgJITLevel, 
			INSTR_TIME_GET_DOUBLE(ji.generation_counter), INSTR_TIME_GET_DOUBLE(ji.inlining_counter),
			INSTR_TIME_GET_DOUBLE(ji.optimization_counter), INSTR_TIME_GET_DOUBLE(ji.emission_counter),
			INSTR_TIME_GET_DOUBLE(total_time), plantime, totaltime);
	...
	if(enableME)
	{
		...
		if(min_total_time == 0 || min_total_time > totaltime)
		{
			min_total_time	=	totaltime;

			for(int i = 0; i < currentExprNum; i++)
			{
				bestJITLevels[i]	=	currentExprJITLevels[i];
			}
		}
		...
		else if((Reserved3 >= 3) && (min_expr_time == 0 || min_expr_time > totaltime))
		{
			min_expr_time	=	totaltime;

			if(Reserved2 < currentExprNum)
				bestCurrentJITLevels[Reserved2]	=	currentExprJITLevels[Reserved2];

			if(enableOPT3)
			{
				for(int i = 0; i < currentExprNum; i++)
				{
					if(i != Reserved2 && bestCurrentJITLevels[i] != 0 && bestCurrentJITLevels[i] != 3)
					{
						bestCurrentJITLevels[i]	=	0;
					}
				}
			}	
		}
...
ExplainPrintJIT(ExplainState *es, int jit_flags, JitInstrumentation *ji)
{
	...
	if(jit_flags & PGJIT_INLINE)
		pgJITLevel = 2;

	if(jit_flags & PGJIT_OPT3)
		pgJITLevel = 3;
